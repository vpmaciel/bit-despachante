...
final class PedidoPlacaImp implements PedidoPlacaDao {
	...
	@Override
	public void deletarRegistro(PedidoPlaca marca) {
		...
		try {
			...
			entityManager.remove(entityManager.find(PedidoPlaca.class, marca.getId()));
	...
	public Collection<PedidoPlaca> getRegistro() {
		...
		try {
			...
			entityTransaction = entityManager.getTransaction();
			entityTransaction.begin();
			Query query = entityManager.createQuery("select T from Marca T order by T.descricao", PedidoPlaca.class);
	...
	@Override
	public boolean isRegistroValido(PedidoPlaca marca) {
		EntityManager entityManager = null;
		EntityTransaction entityTransaction = null;
		List<PedidoPlaca> marcaList = null;
		try {

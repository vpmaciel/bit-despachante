		...
		if (!e.getValueIsAdjusting()) {
			...
			if (selRows.length > 0) {
				...
				if (table.getSelectedRow() != -1) {
					...
					MainControl.getClienteJan().getClienteCont().atualizarGui();
					MainControl.getClienteJan().setFocusable(true);
					MainControl.getClienteJan().getTabbedPane().setSelectedIndex(0);
				}
			}
		}
	}
